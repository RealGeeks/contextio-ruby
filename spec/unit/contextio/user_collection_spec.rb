...
describe ContextIO::UserCollection do
  let(:api) { double('API', url_for: 'url from api') }

  subject { ContextIO::UserCollection.new(api) }
  ...
  describe "#create" do
    ...
    context "without where constraints" do
      ...
      it "returns a User" do
        expect(subject.create(email: 'hello@email.com')).to be_a(ContextIO::User)
      end
    ...
    context "with email in the where constraints" do
      subject { ContextIO::UserCollection.new(api).where(email: 'hello@email.com')}
